...
//Если хочеш использовать 4бит шину тогда расскоментируй #define LCD_4bit
//******************************************
#define LCD_4bit
//***********************************************
...
#define LCD_D4	4	//define MCU pin connected to LCD D4 пины можно располагать не с 7го
#define LCD_D5	5	//define MCU pin connected to LCD D5 а например с 5го по 2й. Тоже будет работать.
#define LCD_D6	6	//define MCU pin connected to LCD D6
#define LCD_D7	7	//define MCU pin connected to LCD D7


//Порт передачи данных в дисплей
#define LDPIN PINC
...
//Порт контроля дисплея, E,RS,R/W
...
void LCDsendChar(uint8_t); //послать символ
void LCDsendCommand(uint8_t); //комманда
void LCDinit(void); //Инициализация LCD
void LCDclr(void); //очистить LCD
void LCDhome(void); //установить курсор в начало
void LCDstring(uint8_t*, uint8_t); //отправить string в LCD
void LCDGotoXY(uint8_t, uint8_t); //перейти в X Y позицию
void CopyStringtoLCD(const uint8_t*, uint8_t, uint8_t); //копировать из флеш string в LCD позиция x,y
void LCDdefinechar(const uint8_t *,uint8_t); //Записать символ в LCD CGRAM 
void LCDshiftRight(uint8_t); //Здвиг символов в право
void LCDshiftLeft(uint8_t); //Здвиг символов в лево 
void LCDcursorOn(void); //Подчеркнутый курсор ВКЛ
void LCDcursorOnBlink(void); //Мигаюший курсор ВКЛ
void LCDcursorOFF(void); //Курсор Выкл 
void LCDblank(void); //LCD blank but not cleared
void LCDvisible(void); //LCD visible
void LCDcursorLeft(uint8_t); //Shift cursor left by n
void LCDcursorRight(uint8_t); //shif cursor right by n
void LCDprogressBar(uint8_t progress, uint8_t maxprogress, uint8_t length); //прогресс бар 
void LCDsendString(char *s); //отправить стринг 
